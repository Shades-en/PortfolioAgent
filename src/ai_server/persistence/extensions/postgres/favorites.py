"""Postgres-backed session repository extension with favorites support."""
...
class PostgresSessionRepositoryWithFavorites:
    ...
    async def list_starred_by_client_id(self, client_id: str) -> list[SessionRecord]:
        sessions = await self.session_model.list_starred_by_client_id(client_id=client_id)
