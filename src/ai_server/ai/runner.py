...
class Runner:
        ...
        async with async_spanner(
            ...
            try:
                while not turn_completed:
                    ...
                    (messages, tool_call), (new_summary, chat_name) = await asyncio.gather(
                ...
                span.set_attribute("steps_taken", self.session_manager.state.step)
                span.set_attribute("tool_calls_made", self.session_manager.state.step > 0)
    ...
    async def run(self, query: str, skip_cache = config.SKIP_CACHE) -> List[dict]:
        ...
        async with async_spanner(
            ...
            output = [msg.model_dump() for msg in result.messages]
            
            # Attach output and update metadata with result info
